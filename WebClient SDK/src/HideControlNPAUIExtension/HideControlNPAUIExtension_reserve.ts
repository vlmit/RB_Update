import { Guid, Visibility, DotNetType } from 'tessa/platform';
import { CardUIExtension, ICardUIExtensionContext } from 'tessa/ui/cards';
import { GridViewModel } from 'tessa/ui/cards/controls';

export class HideControlNPAUIExtension_reserve extends CardUIExtension {
  ...
  public hideControlsArrayInDialog(approvalStagesTable: GridViewModel, controlsAllias: string[]) {
    approvalStagesTable.rowInitializing.addWithDispose((e) => {
      controlsAllias.forEach((controlAllias) => {
        ...
        if (collapsedControl) 
        {
          const approvalStages = e.rowModel.card.sections.tryGet('ApprovalStages');
          approvalStages?.fields.set('TimeLimit', null, DotNetType.Double);
          collapsedControl.controlVisibility = Visibility.Collapsed;
        }
