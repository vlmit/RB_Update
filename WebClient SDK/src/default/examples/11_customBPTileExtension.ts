...
export class CustomBPTileExtension extends TileExtension {
  ...
  private static async startCustomBP(): Promise<void> {
    ...
    // флажок необходимо добавлять через createTypedField, чтобы на сервере правильно отрезолвить тип
    editor.saveCard(context, {
      '.startProcess': createTypedField('TestProcess', DotNetType.String)
    });
  }

  private static enableIfCard(e: TileEvaluationEventArgs): void {
