 ...
 * На левую панель добавляет групповой тайл - "Запросы".
 * По клику показываются дочерние тайлы - "Запрос карточки" и "Запрос представления".
 *
 * При клике на "Запрос карточки" происходит вызов сервера, получается карточка прав, и необработанное
 * содержимое показывается в модальном окне.  Аналогично, с помощью "CardService" можно делать и другие
 * реквесты (request, get, store и т.д.).
 *
 * При клике на "Запрос представления" происходит вызов сервера, получаются данные списка контрагентов,
 * имена которых не равны null и необработанное содержимое показывается в модальном окне.
 */
export class RequestTileExtension extends TileExtension {
  ...
  private static async cardRequestCommand(): Promise<void> {
