...
export class MakeViewTaskHistoryUIExtension extends CardUIExtension {
  ...
  executeInitializedAction = async (typeContext: TypeExtensionContext) => {
    ...
    if (taskHistoryView.table) {
      ...
      table.rowContextMenuGenerators.push(ctx => {
        const resultColumn = table.columns.find(x => x.columnName === 'Result');
        ...
        if (resultColumn) {
          ctx.menuActions.push(
            MenuAction.create({
              name: 'ShowHideResult',
              caption: resultColumn.visibility
                ? '$UI_Cards_TaskHistory_HideResult'
                : '$UI_Cards_TaskHistory_ShowResult',
              action: () => (resultColumn.visibility = !resultColumn.visibility)
            })
          );
        }
    ...
    const multiSelect = taskHistoryView.topItems.find(x => x instanceof MultiSelectButtonViewModel);
    if (multiSelect) {
    ...
    quickSearch.initiailize();
    taskHistoryView.topItems.splice(0, 0, quickSearch);

    const groupButton = new ViewControlBaseButtonViewModel(taskHistoryView);
    groupButton.initiailize();
    groupButton.icon = 'icon-thin-407';
    ...
    taskHistoryView.topItems.push(groupButton);
  };
  ...
  private copyToClipboard(row: ITableRowViewModel) {
    let sb = '';
    for (let column of row.grid.columns.filter(x => x.visibility)) {
      if (sb) {
        sb += '\n';
      }
