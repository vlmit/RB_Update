...
namespace Tessa.Test.Default.Shared.Kr
{
    ...
    public static class KrAssert
    {
        ...
        public static void IsNoneStageActive(Card card)
        {
            ...
            Assert.That(
                rows.Select(p => p.Get<int>(KrConstants.KrStages.StageStateID)),
            ...
            KrStageState expectedState)
        {
            ...
            var state = (KrStageState) row.Get<int>(KrConstants.StateID);
            ...
            StageTypeDescriptor descriptor)
        {
            ...
            var row = stages
                .Rows
                .FirstOrDefault(p => p.Get<int>(KrConstants.KrStages.StageStateID) == KrStageState.Active.ID);
            ...
            string stageName)
        {
            ...
            var rows = stages
                .Rows
                .Where(p => p.Get<int>(KrConstants.KrStages.StageStateID) == KrStageState.Active.ID);
            ...
            KrStageState state)
        {
            ...
            Assert.That(
                ...
                () => $"Expected state ID is \"{state.ID}\" at stage \"{stageName}\" but got \"{row[KrConstants.KrStages.StageStateID]}\".");
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Assert.That(
                task.Planned,
                Is.Not.Null,
                () => $"Planned is required. Task: {CardTaskToString(task)}.");
        ...
        private static string StageRowToString(CardRow row)
        {
            return
                ...
                $"State = {row.Get<string>(KrConstants.KrStages.StageStateName)} (ID = {row.Get<int>(KrConstants.KrStages.StageStateID)}), " +
                $"Name = \"{FormattingHelper.FormatNullable(row.TryGet<string>(KrConstants.Name))}\", " +
                $"Order = {row.Get<int>(KrConstants.Order)}, " +
        ...
        private static string CardTasksToString(IEnumerable<CardTask> rows) =>
            EnumerableToString(rows.Select(CardTaskToString));

        private static string CardTaskToString(CardTask cardTask) =>
            $"Type = {FormattingHelper.FormatNullable(cardTask.TypeName)} (ID = {cardTask.TypeID:B})";
