...
namespace Tessa.Test.Default.Shared.Kr
{
    ...
    public static class CardLifecycleCompanionExtensions
    {
        ...
        /// <summary>
        /// Возвращает значение поля <paramref name="fieldName"/> содержащегося в строке удовлетворяющей условию <paramref name="rowPredicate"/> коллекционной или древовидной секции <paramref name="sectionName"/>.
        /// </summary>
        /// <typeparam name="T">Тип значения.</typeparam>
        /// <param name="clc">Объект, содержащий карточку.</param>
        /// <param name="sectionName">Имя секции.</param>
            ...
            return clc
                .GetTaskOrThrow(
                    task =>
                        ...
                        && task.RoleID == roleID,
                    $"Task type ID = {taskTypeID:B}, Role ID = {roleID:B}.",
                    out task);
        }
            ...
            where T : ICardLifecycleCompanion
        {
            ...
            {
                throw new InvalidOperationException($"{nameof(CardTaskHistoryItem.ParentRowID)} is required. Row: {GetCardTaskHistoryItemInfo(childCardTaskHistoryItem)}.");
            }
            ...
            CardTask cardTask)
        {
            return
                ...
                $"Author = {cardTask.AuthorName} ({cardTask.AuthorID:B}), " +
                $"Role = {cardTask.RoleName} (ID = {cardTask.RoleName:B}), " +
        ...
        private static string GetCardTaskHistoryItemInfo(
            CardTaskHistoryItem item)
        {
            return
                $"TypeCaption = {FormattingHelper.FormatNullable(item.TypeCaption)} (ID = {item.TypeID:B}), " +
                $"Author = {item.AuthorName} (ID = {item.AuthorID:B}), " +
                $"Role = {item.RoleName} (ID = {item.RoleID:B}), " +
        ...
        {
            var tasks = clc.GetCardOrThrow().TryGetTasks();
            var tasksStr = tasks is null
                ? FormattingHelper.EmptyText
                : Environment.NewLine + string.Join(Environment.NewLine, tasks.Select(GetTaskInfo));
            ...
            string notFoundMessage)
        {
            ...
            var itemsStr = items is null
                ? FormattingHelper.EmptyText
                : Environment.NewLine + string.Join(Environment.NewLine, items.Select(GetCardTaskHistoryItemInfo));
