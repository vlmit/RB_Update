...
namespace Tessa.Extensions.Default.Shared.Settings
{
    ...
    public sealed class InjectForumCardMetadataExtension : CardTypeMetadataExtension
    {
        ...
        private static void InjectForumTab(CardType sourceType, CardType targetType)
        {
            ...
            var copy = sourceType.DeepClone();
            // для всех форм меняем блоки на глобальные.
            copy.Forms.ReplaceBlocks(sourceType.Forms);
            
            if (targetType.TabOrder == 0)
            {
            ...
            {
                targetType.TabOrder++;
                sourceType.Forms.InsertWithoutCopy(targetType.Forms, 1);
            }
        }
        ...
        public override async Task ModifyTypes(ICardMetadataExtensionContext context)
        {
            ...
            // т.к. у форм при вставке меняется Order, то в качестве глобальных регистрируем их блоки.
            foreach (var form in forumCardType.Forms)
            {
