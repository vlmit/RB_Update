...
namespace Tessa.Extensions.Default.Shared.Workflow.WorkflowEngine
{
    ...
    public static class WorkflowHelper
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var role = await roleRepository.GetRoleAsync(roleID, cancellationToken).ConfigureAwait(false);
            ...
            switch (role?.RoleType)
            {
                ...
                case RoleType.Context:
                    var contextRole = await roleRepository.GetContextRoleAsync(roleID, cancellationToken).ConfigureAwait(false);
                    var users = await roleRepository.GetCardContextUsersAsync(
                        contextRole,
                        cardID,
        ...
        /// <remarks>
        /// Проверка основана на том, что при отправке задания, стандартным способом, посредством метода <see cref="Tessa.Workflow.WorkflowEngineContext.SendTaskAsync(Guid, string, DateTime?, int?, Guid, string, Guid?, Guid?, Action{CardTask}, CancellationToken)"/> задаётся значение <see cref="CardTask.ProcessKind"/> равное <see cref="WorkflowEngineHelper.WorkflowEngineProcessName"/>, если задание не содержит информации об <see cref="CardTask.ProcessKind"/> и <paramref name="card"/> задан, то выполняется её поиск в истории заданий в <see cref="CardTaskHistoryItem.ProcessKind"/>, если и там отсутствует информация или не задано значение <paramref name="card"/>, то проверяется наличие подписки на задание в таблице <b>WorkflowEngineTaskSubscriptions</b>.
        /// </remarks>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (task.ProcessKind is null)
            {
                ...
                if (card != null)
                {
                    ...
                    if (historyItem != null)
                    {
                        processKind = historyItem.ProcessKind;
                        isCheckInDb = false;
                    }
                }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var taskTypeID in taskTypeIDs)
            {
                ...
                foreach (var completionOption in taskType.CompletionOptions.OrderBy(i => i.Order))
                {
