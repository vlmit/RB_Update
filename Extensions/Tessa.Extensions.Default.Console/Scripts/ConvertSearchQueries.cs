...
namespace Tessa.Extensions.Default.Console.Scripts
{
        ...
        ServerConsoleScriptBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                try
                {
                    ...
                    var jsonMetadata = jsonSearchQuery.ToJsonString();
                ...
                {
                    await this.Logger.ErrorAsync($"Error while converting search query ID={id:D}, ViewAlias={viewAlias}. Metadata:{Environment.NewLine}{metadata}");
                    throw;
                }
            }
        }
        ...
        protected override async ValueTask ExecuteCoreAsync(CancellationToken cancellationToken)
        {
            ...
            await ProcessAsync(fromTableSource, toTableSource, noWarn, cancellationToken);
        }
        ...
        protected override async ValueTask ShowHelpCoreAsync(CancellationToken cancellationToken)
        {
            ...
            await this.Logger.WriteLineAsync("-pp:from=_SearchQueries - Specifies table with apropriate search query metadata in exchange format. \"MetadataLegacy\" column is used.");
