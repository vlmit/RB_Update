...
namespace Tessa.Extensions.Chronos.Plugins
{
    ...
    /// </summary>
    [Plugin(
        Name = "RBSED_ProcessDataFromEOS4SP",
        Description = "Плагин создание карточкек и загрузку данных из СЭД EOS4SP",
        Version = 1,
        ConfigFile = ConfigFilePath)]
    public sealed class RBSED_ProcessDataFromEOS4SPUsersExecutors :
        Plugin
    {
        #region Constants
        ...
        /// </summary>
        private const string ConfigFilePath = "configuration/RBSED_ProcessDataFromEOS4SPUsersExecutors.xml";
        ...
        {
            logger.Info("Starting plugin RBSED_ProcessDataFromEOS4SPUsersExecutors");
            ...
            //поменять путь
            var mainDir = new DirectoryInfo(" ");

            foreach (var directory in mainDir.GetDirectories())
            {
                FileParseSEDCard fp = new FileParseSEDCard();
                ...
                foreach (var c in SPCards)
                {
                    ...
                    var checkGUID = await GetExternalGuidAsync(c["UniqueId"], dbScope);
                    ...
                    var executor = await GetExecutorIDAsync(c["Executors"], dbScope);
                    if (executor != null)
                    {
                        card.Sections["DocumentCommonInfo"].Fields["ExecutorID"] = executor;
                        card.Sections["DocumentCommonInfo"].Fields["ExecutorName"] = c["Executors"];
                    }
                    else
                    {
                        card.Sections["DocumentCommonInfo"].Fields["ExecutorID"] = Guid.Parse("3db19fa0-228a-497f-873a-0250bf0a4ccb");
                        card.Sections["DocumentCommonInfo"].Fields["ExecutorName"] = c["Executors"];
                    }
                    ...
                    var UserFromDB = await GetUserInfoAsync(Convert.ToInt32(c["CreatedBy"]), dbScope);

                    if(UserFromDB != null)
                    {
                        logger.Info($"Found user in DB by ExternalId: {c["CreatedBy"]} ID: {UserFromDB.ID} Name: {UserFromDB.Name}");
                        card.Sections["DocumentCommonInfo"].Fields["RegistratorID"] = UserFromDB.ID;
                        card.Sections["DocumentCommonInfo"].Fields["RegistratorName"] = UserFromDB.Name;
                    }
                    else
                    {
                        logger.Info($"Not found user in DB by ExternalId: {c["CreatedBy"]}");
                    }


                    // Обходим директорию с файлами, добавляем файлы в карточку
                    var dir = new DirectoryInfo(@"/home/tessa/tessa/share/EosIntegration/Администрация Прибайкальского района/Files/" + c["UniqueId"]);
                    ...
                    if (dir.Exists)
                    {
                        await using (var fileContainer = await manager.CreateContainerAsync(card))
                        {
                            ...
                            if (!storeResponse.ValidationResult.IsSuccessful())
                            {
                                ...
                                logger.Info("Files for card with SP UID: " + c["UniqueId"] + " attached with error 1010111");
                            }
                            else
                            {
                                logger.Info("Files for card with SP UID: " + c["UniqueId"] + " attached succesful");
                            }
                        }

                    }

                    string cardExtGuid = c["UniqueId"];

                    // Обходим директорию с файлами, добавляем файлы в карточку
                    var dirCom = new DirectoryInfo(@"/home/tessa/tessa/share/EosIntegration/Администрация Прибайкальского района/Commissions/" + c["UniqueId"]);

                    if (dirCom.Exists)
                    {
                        logger.Info($"Crearing commission " + cardExtGuid);
                        var commissionTypeID = Guid.Parse("aa85fd72-fb7e-4e98-a097-bb1d3216f667");
                        var commissionDocTypes = await typesCache.GetDocTypesAsync();
                        var commissionDocType = docTypes.FirstOrDefault(x => x.ID == typeID);

                        FileParseSEDComissions fpComissions = new FileParseSEDComissions();

                        //var comissions = fpComissions.GetSPCardsFromDir(f);
                        var comissions = fpComissions.GetSPCardsFromDir(@"/home/tessa/tessa/share/EosIntegration/Администрация Прибайкальского района/Commissions/" + c["UniqueId"]);
                        logger.Info($"кол-во поручений {comissions.Count}");
                        foreach (var commi in comissions)
                        {
                            foreach (var s in commi)
                            {
                        ...
                        foreach (var comm in comissions) // создаем карточки по каждому поручению в папке
                        {
                            logger.Info($"Обрабатывается карточка" + comm["CommissionId"] + "из папки" + cardExtGuid);

                            //     var checkCommissionGUID = await GetExternalCommissionGuidAsync(cardExtGuid, comm["CommissionId"], dbScope);
                            //     if (checkCommissionGUID != null)
                            //      {
                            //          logger.Info("Card with UniqueId: " + cardExtGuid + comm["CommissionId"] + " is already exist");
                            //          continue;
                            //      }

                            var newCommissionRequest = new CardNewRequest();

                            // проверяем используется ли тип документа в типовом решении

                            if (commissionDocType != null)
                            {
                                newCommissionRequest.CardTypeID = commissionDocType.CardTypeID;
                                newCommissionRequest.Info[KrConstants.Keys.DocTypeID] = commissionTypeID;
                                newCommissionRequest.Info[KrConstants.Keys.DocTypeTitle] = commissionDocType.Caption;
                            }

                            // для создания карточки из типового решения должны быть права создания для заданного типа у скрытого пользователя System
                            CardNewResponse newCommResponse = await cardRepository.NewAsync(newCommissionRequest);

                            ValidationResult newCommResult = newCommResponse.ValidationResult.Build();
                            // логируем сообщения при создании, если они есть
                            logger.LogResult(newCommResult);
                            // если не удалось создать карточку - выходим
                            if (!newCommResult.IsSuccessful)
                            {
                            ...
                            // теперь у нас есть карточка card, в ней можно заполнить нужные поля и добавить файл
                            Card cardComm = newCommResponse.Card;
                            var cardIDComm = Guid.NewGuid();
                            cardComm.ID = cardIDComm;

                            //if (!String.IsNullOrEmpty(c["DocUID"]) || !String.IsNullOrWhiteSpace(c["DocUID"]))
                            //{
                            cardComm.Sections["DocumentCommonInfo"].Fields["ExternalGuidForTask"] = cardExtGuid;//c["DocUID"];
                                                                                                                //}
                            if (!String.IsNullOrEmpty(comm["CommissionTitle"]) || !String.IsNullOrWhiteSpace(comm["CommissionTitle"]))
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["Subject"] = comm["CommissionTitle"];
                            }
                            cardComm.Sections["DocumentCommonInfo"].Fields["FullNumber"] = comm["CommissionId"];

                            if (!String.IsNullOrEmpty(comm["CommissionCreated"]) || !String.IsNullOrWhiteSpace(comm["CommissionCreated"]))
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["CreationDate"] = DateTime.ParseExact(comm["CommissionCreated"], "M/d/yyyy h:mm:ss", CultureInfo.InvariantCulture); //RegDate
                            }

                            if (!String.IsNullOrEmpty(comm["CommissionDueDate"]) || !String.IsNullOrWhiteSpace(comm["CommissionDueDate"]))
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["TaskDeadline"] = DateTime.ParseExact(comm["CommissionDueDate"], "M/d/yyyy h:mm:ss", CultureInfo.InvariantCulture); //RegDate
                            }

                            var commissionAuthorID = await GetCommissionsUserCreatedByAsync(comm["CommissionAuthor"], dbScope);
                            if (commissionAuthorID != null)
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["AuthorName"] = comm["CommissionAuthor"];
                                cardComm.Sections["DocumentCommonInfo"].Fields["AuthorID"] = commissionAuthorID;
                            }
                            else
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["AuthorName"] = comm["CommissionAuthor"];
                            ...
                            if (!String.IsNullOrEmpty(comm["Controller"]) || !String.IsNullOrWhiteSpace(comm["Controller"]))
                            {
                                var controller = await GetUserCreatedByAsync(comm["Controller"], dbScope);
                                if (controller != null)
                                {
                                    cardComm.Sections["DocumentCommonInfo"].Fields["ControllerName"] = comm["Controller"];
                                    cardComm.Sections["DocumentCommonInfo"].Fields["ControllerID"] = controller;
                                }
                                else
                                {
                                    cardComm.Sections["DocumentCommonInfo"].Fields["ControllerName"] = comm["Controller"];
                                    cardComm.Sections["DocumentCommonInfo"].Fields["ControllerID"] = Guid.Parse("3db19fa0-228a-497f-873a-0250bf0a4ccb");
                                }
                            }

                            // cardComm.Sections["DocumentCommonInfo"].Fields["TreeLevel"] = comm["TreeLevel"];
                            cardComm.Sections["DocumentCommonInfo"].Fields["TreeIndex"] = comm["TreeIndex"];
                            cardComm.Sections["DocumentCommonInfo"].Fields["IsMigration"] = true;
                            //card.Sections["DocumentCommonInfo"].Fields["CurrentStateSP"] = c["StateId"];
                            /* switch (comm["CommissionControl"])
                             {
                                 case "0"://ok
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlID"] = 0;
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlName"] = "Нет контроля";
                                     break;
                                 case "1"://ok
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlID"] = 1;
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlName"] = "На контроле";
                                     break;
                                 case "2"://
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlID"] = 2;
                                     cardComm.Sections["DocumentCommonInfo"].Fields["CommissionControlName"] = "Снято с контроля";
                                     break;
                             }*/

                            //   string parentIndex = null;
                            //           if (comm["TreeLevel"] != "0" && comm["TreeIndex"].Length > 10)
                            //         {
                            //     var temp = comm["TreeIndex"];
                            //             parentIndex = temp.Remove(temp.Length - 10);
                            //         }
                            // cardComm.Sections["DocumentCommonInfo"].Fields["ParentTreeIndex"] = parentIndex;

                            if (!String.IsNullOrEmpty(comm["ExecutorsLogin"]) || !String.IsNullOrWhiteSpace(comm["ExecutorsLogin"]))
                            {
                                RBExecutorRow executorComm = await GetExecutorInfoAsync(comm["ExecutorsLogin"], dbScope);

                                CardSection controlTaskDecisions = cardComm.Sections["ControlTaskDecisions"];

                                CardRow row = controlTaskDecisions.Rows.Add();
                                row.RowID = Guid.NewGuid();
                                var parentRow = row.RowID;
                                row["Question"] = comm["CommissionDescription"];
                                row.State = CardRowState.Inserted;

                                if (executor != null)
                                {
                                    CardSection controlTaskPerformers = cardComm.Sections["ControlTaskPerformers"];
                                    CardRow rowP = controlTaskPerformers.Rows.Add();
                                    rowP.RowID = Guid.NewGuid();
                                    rowP["UserID"] = executorComm.ID;
                                    rowP["UserName"] = executorComm.Name;
                                ...
                                else
                                {
                                    ...
                                    rowP["UserName"] = c["ExecutorsLogin"];
                                    rowP["ParentRowID"] = parentRow;
                                    rowP.State = CardRowState.Inserted;
                                }
                            }
                            ...
                            if (MainCardID != null)
                            {
                                cardComm.Sections["DocumentCommonInfo"].Fields["ParentID"] = MainCardID.ID;
                                cardComm.Sections["DocumentCommonInfo"].Fields["ParentDescription"] = MainCardID.FullNumber + " " + MainCardID.Subject;
                            }
                            //} 
                            logger.Info("Сохраняется поручение к карточке " + cardExtGuid);

                            var storeRequest = await cardRepository.StoreAsync(new CardStoreRequest { Card = cardComm }, cancellationToken);
                    ...
                    logger.Info($"Создана карточка по метаданным {c["UniqueId"]}");
                }
            }
            ...
            logger.Info("Shutting down RBSED_ProcessDataFromEOS4SPUsersExecutors");
        }

        private async Task<RBUser> GetUserInfoAsync(int externalID, IDbScope dbScope)
        {
