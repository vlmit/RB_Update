...
namespace Tessa.Extensions.Server.Web.Services
{
        ...
        Controller
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // сервер сообщает версию только после того, как сессия была успешно открыта, т.е. это пользователь системы, а не случайный человек
            this.Response.Headers[SessionHttpRequestHeader.Version] = BuildInfo.GetVersionObject().ToString();

            return this.Ok(token.SerializeToXml(new SessionSerializationOptions { Mode = SessionSerializationMode.Auth }));
        }
        ...
        public async Task<ActionResult<string>> GetDataWithoutCheckingToken(
            ...
            CancellationToken cancellationToken = default)
        {
            await using (SessionContext.Create(Session.CreateSystemToken(SessionType.Server, this.serverSettings)))
            {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // для того, чтобы ответ на запрос содержал информацию по типам данных, надо вызвать метод-расширение TypedJson
