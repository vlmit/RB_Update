...
namespace Tessa.Extensions.Default.Client.UI.KrProcess
{
        ...
        CardUIExtension
    {
        ...
        private readonly Collection<IDisposable> finalizedObjects;
            ...
            ICardMetadata cardMetadata)
        {
            ...
            Check.ArgumentNotNull(formatterContainer, nameof(formatterContainer));
            Check.ArgumentNotNull(session, nameof(session));
            Check.ArgumentNotNull(viewService, nameof(viewService));
            Check.ArgumentNotNull(uiContainer, nameof(uiContainer));
            ...
            this.finalizedObjects = new Collection<IDisposable>();
        }
        ...
        private async void AddStageDialogHandlerAsync(object sender, GridRowAddingEventArgs e)
        {
            ...
            if (hasGroup)
            {
                ...
                {
                    var groupID = krStageTemplates.RawFields.TryGet<Guid?>(KrConstants.StageGroupID);
                    if (groupID.HasValue)
                    {
            ...
            using (e.Defer())
            {
                ...
                e.Row[KrConstants.Name] = LocalizationManager.Localize(viewModel.SelectedType.DefaultStageName);
            ...
            GridRowEventArgs e)
        {
            ...
            e.Window.Title = row.State == CardRowState.Inserted && KrProcessSharedHelper.RuntimeCard(e.CardModel.Card.TypeID)
                ? LocalizationManager.Format("$UI_StageRowWindowTitle", stageTypeCaption, row.TryGet<string>(KrConstants.StageGroupName))
            ...
            GridRowEventArgs args)
        {
            using (args.Defer())
            {
                await this.RunHandlersAsync(args, static (h, ctx) => h.Initialize(ctx));
            }
        }
            ...
            GridRowValidationEventArgs args)
        {
            using (args.Defer())
            {
                await this.RunHandlersAsync(args, static (h, ctx) => h.Validate(ctx));
            }
        }
            ...
            GridRowEventArgs args)
        {
            using (args.Defer())
            {
                await this.RunHandlersAsync(args, static (h, ctx) => h.Finalize(ctx));
            }
        }
        ...
        private static void BindTimeFieldsRadio(
            object sender,
            GridRowEventArgs args) => args.Row.FieldChanged += OnTimeFieldChanged;
            ...
            foreach (Guid id in list)
            {
                if (id == stageTypeID)
                {
                    if (mode == PerformerUsageMode.Single
                        ...
                        && !perfSec.Rows.Any(p => p.Get<Guid>(KrConstants.StageRowID) == row.RowID && p.State != CardRowState.Deleted))
                    {
        ...
        {
            if (this.finalizedObjects is not null)
            {
        ...
        {
            if (args.Action == GridRowAction.Deleting
                && KrProcessSharedHelper.SkipStage(args.Row))
            {
                var currentRowViewModel = args.Control.SelectedRows
                    .FirstOrDefault(i => i.Model == args.Row);
