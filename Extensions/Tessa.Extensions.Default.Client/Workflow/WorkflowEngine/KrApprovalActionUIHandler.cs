    ...
    public sealed class KrApprovalActionUIHandler : WorkflowActionUIHandlerBase
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isReadOnly)
            {
                ...
                additionalPerformersSettingsSection.FieldChanged += (s, e) =>
                {
                    switch (e.FieldName)
                    {
                        ...
                        case KrApprovalActionAdditionalPerformersSettingsVirtual.IsAdditionalApprovalFirstResponsible when e.FieldValue is bool isAdditionalApprovalFirstResponsible:
                            ...
                            {
                                var minOrder = filteredAdditionalPerformerRows.Min(i => i.Fields.Get<int>(KrApprovalActionAdditionalPerformersVirtual.Order));
                ...
                var additionalPerformersBlock = ((ContainerViewModel) cardModel.Controls[WorkflowConstants.UI.AdditionalApprovalContainer]).Form.Blocks.Single(i => string.Equals(i.Name, WorkflowConstants.UI.AdditionalApprovalBlock, StringComparison.Ordinal));
                var additionalPerformers = (AutoCompleteTableViewModel) additionalPerformersBlock.Controls.Single(i => string.Equals(i.Name, WorkflowConstants.UI.AdditionalApprovers, StringComparison.Ordinal));

                additionalPerformers.ValueSelected += (sender, e) =>
                {
                ...
                additionalPerformers.ValueDeleted += (sender, e) =>
                {
                ...
                ((INotifyPropertyChanged) performers.ItemsSource).PropertyChanged += (sender, e) =>
                 {
                     ...
                     {
                         var isSelectedPerformer = GetSelectedMainApproverRow() != null;
                         SelectedPerformerChanged(isSelectedPerformer);
                         if (ChangeVisibility(additionalPerformersBlock, isSelectedPerformer ? Visibility.Visible : Visibility.Collapsed))
                         {
                             cardModel.MainForm.Rearrange();
                         }
                     }
                 };
                ...
                performersSection.Rows.ItemChanged += (sender, e) =>
                {
                    switch (e.Action)
                    {
                        case ListStorageAction.Remove:
                            var isRearrangeBlockRequered = false;
                            if (IsComputedRole(e.Item))
                            {
                                if (ChangeVisibility(addComputedRoleLink, Visibility.Visible))
                                {
                                    isRearrangeBlockRequered = true;
                                }
                            }
                            ...
                            else if (isRearrangeBlockRequered)
                            {
                                addComputedRoleLink.Block.Form.RearrangeChildren();
                            }
            ...
            void SelectedPerformerChanged(bool isSelectedPerformer)
            {
                ...
                var orderredAdditionalPerformerRows = GetAdditionalPerformerRows(additionalPerformersSection.Rows, currentMainApproverRowID)
                    .OrderBy(i => i.Fields.Get<int>(KrApprovalActionAdditionalPerformersVirtual.Order))
                    .ToArray();
        ...
        private void MainSection_FieldChanged(object sender, CardFieldChangedEventArgs e)
        {
            ...
            switch (e.FieldName)
            {
                case KrApprovalActionVirtual.Period when e.FieldValue != null:
                    mainSectionInner.Fields[KrApprovalActionVirtual.Planned] = default;
                    break;
                case KrApprovalActionVirtual.Planned when e.FieldValue != null:
        ...
        private void EditInterjectOptions_FieldChanged(object sender, CardFieldChangedEventArgs e)
        {
            ...
            switch (e.FieldName)
            {
                case KrWeEditInterjectOptionsVirtual.Period when e.FieldValue != null:
                    mainSectionInner.Fields[KrWeEditInterjectOptionsVirtual.Planned] = default;
                    break;
                case KrWeEditInterjectOptionsVirtual.Planned when e.FieldValue != null:
