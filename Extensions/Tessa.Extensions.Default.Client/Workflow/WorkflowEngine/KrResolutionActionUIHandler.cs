...
namespace Tessa.Extensions.Default.Client.Workflow.WorkflowEngine
{
    ...
    public sealed class KrResolutionActionUIHandler : WorkflowActionUIHandlerBase
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isReadOnly)
            {
                mainSection.FieldChanged += (sender, e) =>
                {
                    switch (e.FieldName)
                    {
                        case KrResolutionActionVirtual.Period when e.FieldValue != null:
                            mainSection.Fields[KrResolutionActionVirtual.Planned] = default;
                            break;
                        case KrResolutionActionVirtual.Planned when e.FieldValue != null:
                            mainSection.Fields[KrResolutionActionVirtual.Period] = default;
                            break;
                        case KrResolutionActionVirtual.WithControl:
                            ChangeControllerVisibility(controllerAutoComplete, cardModel.MainForm, e.FieldValue is bool value && value);
                            break;
                        case KrResolutionActionVirtual.IsMassCreation:
                            ChangeIsMajorPerformerChecked(mainSection, (e.FieldValue is bool isMassCreation) && isMassCreation);
                            break;
                    }
                };
                ...
                performersSection.Rows.ItemChanged += (sender, e) =>
                {
                    ...
                    switch (e.Action)
                    {
                        case ListStorageAction.Clear:
                            ChangeVisibilityChildren(addComputedRoleLink, true);
                ...
                addComputedRoleLink.CommandClosure.Execute = _ =>
                {
                    ...
                    ChangeVisibilityChildren(addComputedRoleLink, false);
                };
            }

            ChangeVisibilityChildren(
                addComputedRoleLink,
        ...
        private static void ChangeVisibilityChildren(IControlViewModel control, bool isVisible)
        {
        ...
        private static bool HasManyPerformers(IEnumerable<CardRow> rows)
        {
        ...
        private static void ChangeControllerVisibility(IControlViewModel controllerAutoComplete, IFormViewModel form, bool isVisible)
        {
            ChangeVisibilityAndRearrangeChildren(
                controllerAutoComplete,
                form,
