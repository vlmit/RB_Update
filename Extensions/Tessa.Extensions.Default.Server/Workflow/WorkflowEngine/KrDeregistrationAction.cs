    ...
    public sealed class KrDeregistrationAction : KrWorkflowActionBase
    {
        ...
        protected override async Task ExecuteAsync(IWorkflowEngineContext context, IWorkflowEngineCompiled scriptObject)
        {
            ...
            if (context.ValidationResult.IsSuccessful())
            {
                ...
                var prevStateID = krProcessInfo.TryGetValue(KrConstants.Keys.StateBeforeRegistration, out object stateIDObj)
                ...
                await this.SetStateIDAsync(
                    context,
                    prevStateID,
                    await context.CardMetadata.GetDocumentStateNameAsync((KrState)prevStateID, context.CancellationToken),
                    cancellationToken: context.CancellationToken);
            }
        }
