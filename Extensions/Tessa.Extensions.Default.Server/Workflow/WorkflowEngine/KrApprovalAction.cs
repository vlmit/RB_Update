...
namespace Tessa.Extensions.Default.Server.Workflow.WorkflowEngine
{
    ...
    public sealed class KrApprovalAction : KrWorkflowMultiTaskActionBase
    {
        ...
        private const string MainSectionName = KrApprovalActionVirtual.SectionName;

        /// <summary>
        /// Идентификатор типа задания согласования.
        /// </summary>
        private static readonly Guid taskTypeID = DefaultTaskTypes.KrApproveTypeID;

        /// <summary>
        /// Значение по умолчанию для параметра "Длительность, рабочие дни". Данное значение используется только,
        /// если в схеме не указано значение по умолчанию для поля <see cref="KrApprovalActionVirtual.Period"/>.
        /// </summary>
        ...
        /// </summary>
        private static readonly KrState activeState = KrState.Active;

        /// <summary>
        /// Состояние документа при положительном завершении обработки действия.
        /// </summary>
        private static readonly KrState positiveState = KrState.Approved;

        /// <summary>
        /// Состояние документа при отрицательном завершении обработки действия.
        /// </summary>
        private static readonly KrState negativeState = KrState.Disapproved;

        /// <summary>
        /// Идентификатор варианта завершения действия при положительном завершении обработки.
        /// </summary>
        private static readonly Guid positiveActionCompletionOptionID = ActionCompletionOptions.Approved;

        /// <summary>
        /// Идентификатор варианта завершения действия при отрицательном завершении обработки.
        /// </summary>
        private static readonly Guid negativeActionCompletionOptionID = ActionCompletionOptions.Disapproved;
        ...
        /// <summary>
        /// Инициализирует новый экземпляр класса <see cref="KrApprovalAction"/>.
        /// </summary>
        public KrApprovalAction(
            ...
            : base(
                  descriptor: KrDescriptors.KrApprovalDescriptor,
        ...
        public override void PrepareForExecute(WorkflowActionStateStorage actionState, IWorkflowEngineContext context)
        {
            ...
            actionState.Hash.Remove(KrApprovalActionOptionsVirtual.SectionName);
        }
            ...
            using (this.CreateTasksContext(context))
            {
                if (signalType == WorkflowSignalTypes.Default)
                {
                    ...
                    if (!hasTaskIDList)
                    {
                        ...
                        if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.ChangeStateOnStart) == true)
                        {
                        ...
                        if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.IsParallel) == true)
                        {
                            await this.SendApprovalTaskAsync(context, scriptObject, performers);
                        }
                        else
                        {
                            await this.SendApprovalTaskAsync(context, scriptObject, performers.Take(1));
                        }
            ...
            if (task.TypeID == taskTypeID)
            {
                return
                    await this.GetWithPlaceholdersAsync(
                        context,
                        await context.GetAsync<string>(MainSectionName, KrApprovalActionVirtual.Result),
                        task);
            }
            ...
            CardTask delegatedTask)
        {
            ...
            if (originalTask.Settings.TryGet<bool>(NamesKeys.IsDisableAutoApproval))
            {
                delegatedTask.Settings[NamesKeys.IsDisableAutoApproval] = BooleanBoxes.True;
            }
            ...
            Guid taskOptionID)
        {
            ...
            else if (task.TypeID == taskTypeID)
            {
                if (taskOptionID == DefaultCompletionOptions.Approve)
                {
                    actionOptionID = await this.CompleteApprovalTaskAsync(context, scriptObject, task, false);
                }
                else if (taskOptionID == DefaultCompletionOptions.Disapprove)
                {
                    actionOptionID = await this.CompleteApprovalTaskAsync(context, scriptObject, task, true);
                }
                else if (taskOptionID == DefaultCompletionOptions.AdditionalApproval)
                {
                    await this.StartAdditionalApprovalTaskAsync(context, scriptObject, task);
        ...
        private async Task SendApprovalTaskAsync(IWorkflowEngineContext context, IWorkflowEngineCompiled scriptObject, IEnumerable<RoleEntryStorage> roles)
        {
            var periodInDays = (double?) (await context.GetAsync<object>(MainSectionName, KrApprovalActionVirtual.Period)
                ?? (await context.CardMetadata.GetSectionsAsync(context.CancellationToken))[MainSectionName]
                    .Columns[KrApprovalActionVirtual.Period]
                    .DefaultValue)
                ?? PeriodInDaysDefaultValue;
            var planned = await context.GetAsync<DateTime?>(MainSectionName, KrApprovalActionVirtual.Planned);

            var parentTaskRowID = context.Signal.As<WorkflowEngineTaskSignal>().ParentTaskRowID;
            var authorID = await context.GetAsync<Guid?>(MainSectionName, KrApprovalActionVirtual.Author, Names.Table_ID);

            authorID = await context.GetAuthorIDAsync(this.roleRepository, authorID);
            if (!authorID.HasValue)
            {
                return;
            }

            var kindID = await context.GetAsync<Guid?>(MainSectionName, KrApprovalActionVirtual.Kind, Names.Table_ID);
            var kindCaption = await context.GetAsync<string>(MainSectionName, KrApprovalActionVirtual.Kind, Table_Field_Caption);

            var digest = await context.GetAsync<string>(MainSectionName, KrApprovalActionVirtual.Digest);
            ...
            foreach (var performer in roles)
            {
                ...
                if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.CanEditCard) == true)
                {
                    cardTask.Settings[NamesKeys.CanEditCard] = BooleanBoxes.True;
                }

                if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.CanEditAnyFiles) == true)
                {
                    cardTask.Settings[NamesKeys.CanEditAnyFiles] = BooleanBoxes.True;
                }

                if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.IsDisableAutoApproval) == true)
                {
                    cardTask.Settings[NamesKeys.IsDisableAutoApproval] = BooleanBoxes.True;
                }
                ...
                var additionalApprovalPerformers = await context.GetAllRowsAsync(KrApprovalActionAdditionalPerformersVirtual.SectionName);
                if (additionalApprovalPerformers is not null)
                {
                    var isAdvisoryParentTask = await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.IsAdvisory) ?? false;
                    var filteredAdditionalApprovalPerformers = additionalApprovalPerformers.Where(
                        i => WorkflowEngineHelper.Get<Guid>(i, KrApprovalActionAdditionalPerformersVirtual.MainApprover, Names.Table_RowID) == performer.ParentRoleID);
                    foreach (var additionalApprovalPerformer in filteredAdditionalApprovalPerformers)
                    {
                        await this.SendAdditionalApprovalTaskAsync(
                            context,
                            scriptObject,
                            cardTask,
                            WorkflowEngineHelper.Get<Guid>(additionalApprovalPerformer, KrApprovalActionAdditionalPerformersVirtual.Role, Names.Table_ID),
                            WorkflowEngineHelper.Get<string>(additionalApprovalPerformer, KrApprovalActionAdditionalPerformersVirtual.Role, Table_Field_Name),
                            WorkflowEngineHelper.Get<bool>(additionalApprovalPerformer, KrApprovalActionAdditionalPerformersVirtual.IsResponsible),
                            cardTask.AuthorID,
                            default,
            ...
            double? plannedDays)
        {
            ...
            var task =
                await context.SendTaskAsync(
                    ...
                    digest,
                    planned,
                    plannedDays.HasValue ? (int?) Math.Round(plannedDays.Value * TimeZonesHelper.QuantsInDay) : default,
                    roleID,
                    roleName: roleName,
            ...
            taskSections[TaskCommonInfo.Name].Fields[Info] = comment;

            var taskTaskInfoInfoFields = taskSections[KrAdditionalApprovalTaskInfo.Name].Fields;
            taskTaskInfoInfoFields[KrAdditionalApprovalTaskInfo.AuthorRoleID] = parentTask.RoleID;
            taskTaskInfoInfoFields[KrAdditionalApprovalTaskInfo.AuthorRoleName] = parentTask.RoleName;
            ...
            await context.AddToHistoryAsync(task.RowID, WorkflowHelper.GetProcessCycle(context.ProcessInstance.Hash), isAdvisoryParentTask);
        ...
        /// <returns>Идентификатор варианта завершения действия, для которого должны быть обработаны связи выполняющиеся после завершения обработки этого действия или значение по умолчанию для типа, если действие не завершается.</returns>
        private async Task<Guid?> CompleteApprovalTaskAsync(
            IWorkflowEngineContext context,
            IWorkflowEngineCompiled scriptObject,
            CardTask task,
            bool isNegativeResult)
        {
            ...
            var isAdvisory = await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.IsAdvisory) ?? false;
            ...
            if (isAdvisory || isSetCommentTaskResult)
            {
            ...
            {
                if (!isAdvisory && context.ActionInstance.Hash.Get<bool>(NamesKeys.IsNegativeActionResult))
                {
            ...
            else
            {
                if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.IsParallel) != true)
                {
                    if (!isAdvisory && isNegativeResult)
                    {
                        await this.NegativeResultProcessingAsync(context);

                        if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.ExpectAllApprovers) != true)
                        {
                            return negativeActionCompletionOptionID;
                        }
                    }

                    await this.SendApprovalTaskAsync(
                        context,
                        scriptObject,
                        context.ActionInstance.Hash
                            // может быть List<object> или List<Dictionary<string, object>>, поэтому используем ковариантный интерфейс
        ...
        private async Task PositiveResultProcessingAsync(
            ...
            IWorkflowEngineCompiled scriptObject)
        {
            if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.ReturnWhenApproved) == true)
            {
        ...
        private async Task CompleteTaskScriptAsync(
            ...
            bool isSetResult)
        {
            var optionRows = await context.GetAllRowsAsync(context.ActionTemplate.Hash, KrApprovalActionOptionsVirtual.SectionName);
        ...
        private async Task CompleteActionAsync(
            ...
            Guid actionOptionID)
        {
            if (await context.GetAsync<bool?>(MainSectionName, KrApprovalActionVirtual.ChangeStateOnEnd) == true)
            {
                await this.SetStateIDAsync(
                    context,
            ...
            var optionActionRows = await context.GetAllRowsAsync(context.ActionTemplate.Hash, KrApprovalActionOptionsActionVirtual.SectionName);
            ...
            if (optionActionRows?.Any() == true)
            {
                ...
                foreach (var processRow in processRows)
                {
                    var info = new WorkflowTaskNotificationInfo(
                        context,
                        processRow,
                        KrApprovalActionNotificationActionRolesVirtual.Option,
                    ...
                    var linkRows = context.ActionTemplate.Hash.TryGet<IList>(KrApprovalActionOptionLinksVirtual.SectionName);
