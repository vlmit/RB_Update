...
namespace Tessa.Extensions.Default.Server.Workflow.WorkflowEngine
{
    public sealed class KrAcquaintanceAction : WorkflowActionBase
    {
            ...
            IWorkflowEngineCompiled scriptObject)
        {
            ...
            {
                var role = await this.roleRepository.GetRoleAsync(senderID.Value, cancellationToken: context.CancellationToken);
                if (role == null)
                {
                ...
                switch (role.RoleType)
                {
                    ...
                    case RoleType.Context:
                        var contextRole = await this.roleRepository.GetContextRoleAsync(senderID.Value, cancellationToken: context.CancellationToken);
                        var users = await this.roleRepository.GetCardContextUsersAsync(
                            contextRole, 
                            mainCardID, 
