...
namespace Tessa.Extensions.Default.Server.Workflow.KrPermissions
{
    ...
    public sealed class KrPermissionsCacheContainer : IKrPermissionsCacheContainer
    {
        ...
        {
            var result = await cache.Settings.TryGetAlreadyCachedAsync<long?>(VersionKey, cancellationToken);
            if (result.HasValue)
            {
                return result.Value;
            }
            ...
            {
                result = await cache.Settings.TryGetAlreadyCachedAsync<long?>(VersionKey, cancellationToken);
                if (result.HasValue)
                {
                    return result.Value;
                }
        ...
        {
            var result = await cache.Settings.TryGetAlreadyCachedAsync<KrPermissionsCacheObject>(CacheKey, cancellationToken);
            if (result != null)
            {
                return result;
            }
            ...
            using (await asyncLock.EnterAsync(cancellationToken))
            {
                result = await cache.Settings.TryGetAlreadyCachedAsync<KrPermissionsCacheObject>(CacheKey, cancellationToken);
                if (result != null)
                {
                    return result;
                }
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                {
                    prevFileRule.Categories.Add(categoryID.Value);
                }
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            var sections = (await schemeService.GetTablesAsync(cancellationToken)).ToDictionary(x => x.ID, x => x);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                int i = 5; // Список колонок флагов начинается с 5
                foreach (var flag in KrPermissionFlagDescriptors.Full.IncludedPermissions)
                {
                    if (flag.IsVirtual)
                    {
