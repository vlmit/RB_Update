...
namespace Tessa.Extensions.Default.Server.Workflow.KrPermissions
{
    public sealed class KrPermissionsStoreExtension : CardStoreExtension
    {
        ...
        private async Task<KrPermissionFlagDescriptor[]> GetRequiredPermissionsAsync(Guid userID, Card card, IValidationResultBuilder validationResults, CancellationToken cancellationToken)
        {
            ...
            if (card.StoreMode == CardStoreMode.Update)
            {
                ...
                if (containsKrStagesVirtual)
                {
                    ...
                    foreach (var stage in krStagesVirtual.Rows.Where(i => i.State == CardRowState.Deleted))
                    {
                        ...
                        {
                            satellite = await this.krScope.GetKrSatelliteAsync(card.ID, validationResults, cancellationToken);
        ...
        private async Task CheckPermissionsOnCreatingAsync(ICardStoreExtensionContext context, Card storeCard)
        {
            ...
            if (permContext != null)
            {
                context.Info[nameof(KrPermissionsStoreExtension)] = await this.permissionsManager.CheckRequiredPermissionsAsync(
                    permContext,
                    await this.GetRequiredPermissionsAsync(context.Session.User.ID, storeCard, context.ValidationResult, context.CancellationToken));
            }
        }
