...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow.Handlers
{
    ...
    public class DialogStageTypeHandler : StageTypeHandlerBase
    {
        ...
        public abstract class ScriptContextBase
        {
            ...
            {
                Check.ArgumentNotNull(file, nameof(file));
            ...
            ISession session)
        {
            ...
            Check.ArgumentNotNull(compilationCache, nameof(compilationCache));
            Check.ArgumentNotNull(unityContainer, nameof(unityContainer));
            Check.ArgumentNotNull(cardRepositoryDwt, nameof(cardRepositoryDwt));
            ...
            Check.ArgumentNotNull(tasksRevoker, nameof(tasksRevoker));
            Check.ArgumentNotNull(typesCache, nameof(typesCache));
            Check.ArgumentNotNull(cardFileManager, nameof(cardFileManager));
            Check.ArgumentNotNull(cardRepository, nameof(cardRepository));
            Check.ArgumentNotNull(ctcBuilderFactory, nameof(ctcBuilderFactory));
            Check.ArgumentNotNull(session, nameof(session));
        ...
        /// <inheritdoc cref="IKrCompilationCache" path="/summary"/>
        ...
        protected ICardRepository CardRepositoryDwt { get; }
            ...
            IStageTypeHandlerContext context)
        {
            ...
            if (inst is not null)
            {
                ...
                await inst.InvokeExtraAsync(ValidationMethodDescriptor.MethodName, scriptContext);
            }
            ...
            IStageTypeHandlerContext context)
        {
            ...
            if (actionInfo.StoreMode == CardTaskDialogStoreMode.Settings)
            {
                ...
                var savingScriptInstance = await KrProcessHelper.TryCreateScriptInstanceAsync(
                    this.CompilationCache,
                    context.Stage.ID,
                    SourceIdentifiers.StageAlias,
                    context.CancellationToken);
                ...
                {
                    await HandlerHelper.InitScriptContextAsync(this.UnityContainer, savingScriptInstance, context);
                    await savingScriptInstance.InvokeExtraAsync(SavingMethodDescriptor.MethodName, savingScriptContext);
                }
            ...
            var validationScriptInstance = await KrProcessHelper.TryCreateScriptInstanceAsync(
                this.CompilationCache,
                context.Stage.ID,
                SourceIdentifiers.StageAlias,
                context.CancellationToken);
            ...
            {
                await HandlerHelper.InitScriptContextAsync(this.UnityContainer, validationScriptInstance, context);
                await validationScriptInstance.InvokeExtraAsync(ValidationMethodDescriptor.MethodName, scriptContext);
            }
            ...
            IStageTypeHandlerContext context)
        {
            ...
            {
                await HandlerHelper.InitScriptContextAsync(this.UnityContainer, inst, context);
                await inst.InvokeExtraAsync(SavingMethodDescriptor.MethodName, scriptContext);
            }
        ...
        /// <param name="actionInfo">Информация о результате завершения дилога.</param>
        /// <param name="context">Контекст обработчика этапа.</param>
            ...
            if (buttonsSettings != null)
            {
                foreach (var buttonStorage in buttonsSettings.Cast<Dictionary<string, object>>())
                {
                    var button = new CardTaskDialogButtonInfo
                    {
                        ...
                        Order = buttonStorage.TryGet<int>(KrConstants.Order),
                    };
