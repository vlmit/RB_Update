...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow.Handlers
{
    ...
    public class CreateCardStageTypeHandler : StageTypeHandlerBase
    {
        ...
        /// </summary>
        /// <param name="cardRepositoryEwt">Репозиторий для управления карточками (<see cref="CardRepositoryNames.ExtendedWithoutTransaction"/>).</param>
        /// <param name="cardRepositoryDwt">Репозиторий для управления карточками (<see cref="CardRepositoryNames.DefaultWithoutTransaction"/>).</param>
        /// <param name="tokenProvider">Объект, обеспечивающий создание и валидацию токена безопасности для типового решения.</param>
        /// <param name="fileManager">Объект, который управляет объектами контейнеров <see cref="ICardFileContainer"/>, объединяющих карточку с её файлами (<see cref="CardRepositoryNames.ExtendedWithoutTransaction"/>).</param>
        ...
        /// <summary>
        /// Возвращает или задаёт репозиторий для управления карточками (<see cref="CardRepositoryNames.ExtendedWithoutTransaction"/>).
        /// </summary>
        protected ICardRepository CardRepositoryEwt { get; set; }

        /// <summary>
        /// Возвращает или задаёт репозиторий для управления карточками (<see cref="CardRepositoryNames.DefaultWithoutTransaction"/>).
        /// </summary>
        ...
        /// <summary>
        /// Возвращает или задаёт объект, который управляет объектами контейнеров <see cref="ICardFileContainer"/>, объединяющих карточку с её файлами (<see cref="CardRepositoryNames.ExtendedWithoutTransaction"/>).
        /// </summary>
            ...
            Func<CardStoreRequest, CancellationToken, ValueTask> modifyRequestActionAsync = default)
        {
            ...
            await using (var container = await this.FileManager.CreateContainerAsync(card, cancellationToken: context.CancellationToken))
            {
                var storeResponse = await container.StoreAsync(async (c, request, ct) =>
                {
                    ...
                    if (modifyRequestActionAsync != null)
                    {
            ...
            Card card)
        {
            ...
            var startProcessResponse = await this.CardRepositoryEwt.StoreAsync(startProcessRequest, context.CancellationToken);
