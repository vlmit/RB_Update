...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow.Handlers
{
    ...
    public class ForkStageTypeHandler : ForkStageTypeHandlerBase
    {
        ...
        /// <summary>
        /// Предоставляет контекст сценария <see cref="AfterNestedMethodName"/>.
        /// </summary>
        // ReSharper disable once ClassCanBeSealed.Global
        ...
        /// <summary>
        /// Ключ, по которому в <see cref="Stage.InfoStorage"/> содержится коллекция объектов содержазих информацию по веткам этапа "Ветвление".
        /// </summary>
        ...
        /// <returns>Контекст сценария <see cref="AfterNestedMethodName"/>.</returns>
            ...
            ListStorage<BranchInfo> branchInfos)
        {
            ...
            {
                SecondaryProcess = await this.ProcessCache.GetSecondaryProcessAsync(binfo.SecondaryProcessID, context.CancellationToken),
        ...
        /// </summary>
        /// <param name="context">Контекст обработчика этапа.</param>
        /// <param name="ctx">Контекст сценария <see cref="AfterNestedMethodName"/>.</param>
        /// <param name="branchInfos">Коллекция содержащая информацию о ветках этапа.</param>
            ...
            IEnumerable<IDictionary<string, object>> secProcsRows)
        {
            ...
            foreach (var secondaryProcessRow in secProcsRows)
            {
                ...
                if (complete)
                {
                    ...
                    if ((await this.RunScriptAsync(context, branchInfo, result.ProcessInfo, branchInfos)).SkipForkAndContinueRoute)
                    {
                        return StageHandlerResult.CompleteResult;
                    }
                }
