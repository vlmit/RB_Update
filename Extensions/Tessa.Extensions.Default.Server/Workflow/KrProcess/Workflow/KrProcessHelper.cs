...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow
{
    public static class KrProcessHelper
    {
        ...
        public static void SetStageDefaultValues(CardValueResponseBase responseWithCard)
        {
            ...
            {
                stageRow[KrConstants.KrStages.StageStateID] = Int32Boxes.Box(KrStageState.Inactive.ID);
                stageRow[KrConstants.KrStages.StageStateName] = KrStageState.Inactive.TryGetDefaultName();
            }
        }
        ...
        public static void SetInactiveStateToStages(Card card)
        {
            ...
            if (card.TryGetStagesSection(out var stagesSec))
            {
                ...
                {
                    row[KrConstants.KrStages.StageStateID] = Int32Boxes.Box(KrStageState.Inactive.ID);
        ...
        /// </summary>
        /// <param name="compilationCache">Кэш последней сборки сгенерированного кода.</param>
        /// <param name="id">Идентификатор объекта, содержащего инициализируемый сценарий.</param>
        /// <param name="alias">Алиас объекта, содержащего инициализируемый сценарий.</param>
        /// <param name="validationResult">Объект выполняющий построение результатов валидации.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Пустой экземпляр класса сценария маршрута.</returns>
        /// <exception cref="ProcessRunnerInterruptedException">Исключение, возникающее при прерывании выполнения <see cref="IKrProcessRunner"/>.</exception>
        public static async ValueTask<IKrScript> CreateScriptInstanceAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (compilationResult.Result.Assembly is null)
            {
            ...
            catch (MissingCompiledClassException e)
            {
                logger.LogException(e);
                validationResult.AddException(nameof(KrProcessHelper), e);
                throw new ProcessRunnerInterruptedException();
            }
        }

        /// <summary>
        /// Создаёт новый пустой экземпляр класса сценария маршрута. Не создаёт исключений, если не удалось найти тип создаваемого экземпляра класса. Информация об ошибке записывается в лог сервера приложений.
        /// </summary>
        /// <param name="compilationCache">Кэш последней сборки сгенерированного кода.</param>
        /// <param name="id">Идентификатор объекта, содержащего инициализируемый сценарий.</param>
        /// <param name="alias">Алиас объекта, содержащего инициализируемый сценарий.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Пустой экземпляр класса сценария маршрута.</returns>
        public static async ValueTask<IKrScript> TryCreateScriptInstanceAsync(
            IKrCompilationCache compilationCache,
            Guid id,
            string alias,
            CancellationToken cancellationToken = default)
