...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow
{
        ...
        IKrProcessLauncher
    {
            ...
            IKrCompilationCache krCompilationCache)
        {
            ...
            Check.ArgumentNotNull(executorFunc, nameof(executorFunc));
            Check.ArgumentNotNull(krScope, nameof(krScope));
            Check.ArgumentNotNull(cardRepository, nameof(cardRepository));
            Check.ArgumentNotNull(cardRepositoryEwt, nameof(cardRepositoryEwt));
            ...
            Check.ArgumentNotNull(taskHistoryManager, nameof(taskHistoryManager));
            Check.ArgumentNotNull(processCache, nameof(processCache));
            Check.ArgumentNotNull(secondaryProcessExecutionEvaluator, nameof(secondaryProcessExecutionEvaluator));
            Check.ArgumentNotNull(session, nameof(session));
            Check.ArgumentNotNull(eventManager, nameof(eventManager));
            ...
            Check.ArgumentNotNull(krCompilationCache, nameof(krCompilationCache));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var level = this.krScope.EnterNewLevel(this.WithReaderLocks());
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!resurrection)
            {
                ...
                var ctx = new KrExecutionContext(
                    ...
                    krCompilationCache: this.krCompilationCache,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (selectCardTypeInfo)
            {
                cardInfo.CardType = await CardComponentHelper.TryGetCardTypeAsync(
                    await this.GetCardTypeInfoAsync(
                        cardID,
                        cancellationToken),
                    this.cardMetadata,
                    cancellationToken);
        ...
        /// <returns>Идентификатор типа карточки.</returns>
