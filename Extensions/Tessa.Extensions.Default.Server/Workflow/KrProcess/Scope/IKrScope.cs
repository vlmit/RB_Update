...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Scope
{
    ...
    public interface IKrScope
    {
        ...
        /// При загрузке объекта карточки (для инкремента версии, если карточка не загружена ранее)
        ...
        /// </param>
        /// <param name="withoutTransaction">Значение <see langword="true"/>, если карточка, при выполнении вне контекста <see cref="KrScopeContext"/>, должна быть загружена без транзакции и без взятия блокировки на чтение карточки, иначе - <see langword="false"/>. При выполнении в контексте <see cref="KrScopeContext"/> карточка всегда загружается вне транзакции.</param>
        ...
        /// <param name="cancellationToken">Объект, посредством которого можно отменить выполнения асинхронной задачи.</param>
        /// <exception cref="InvalidOperationException">Код вызван вне <see cref="KrScopeContext"/>.</exception>
        Task StoreSatelliteExplicitlyAsync(
            Guid mainCardID,
