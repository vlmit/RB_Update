...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess
{
    ...
    public readonly struct KrProcessSectionMapper
    {
            ...
            bool defaultAddDestinationSection = false)
        {
            ...
            Check.ArgumentNotNull(destination, nameof(destination));
        ...
        {
            Check.ArgumentNotNullOrEmpty(sourceSectionAlias, nameof(sourceSectionAlias));
            Check.ArgumentNotNullOrEmpty(destinationSectionAlias, nameof(destinationSectionAlias));
            ...
            for (var i = 0; i < stagePositions.Length; i++)
            {
                ...
                if (srcRow is null)
                {
                    if (pos.HiddenRow is not null)
                    {
                        ...
                        row.Fields[KrConstants.Order] = Int32Boxes.Box(order);
                    ...
                    else
                    {
                        ...
                        {
                            destRow.Fields[KrConstants.Order] = Int32Boxes.Box(order);
                        }
                    }
                }
                else
