...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Serialization
{
        ...
        IKrStageSerializer
    {
        ...
        {
            Check.ArgumentNotNull(krStagesSection, nameof(krStagesSection));
            Check.ArgumentNotNull(updatedSections, nameof(updatedSections));
            ...
            IDictionary<string, object> settingsStorage)
        {
            ...
            Check.ArgumentNotNull(settingsStorage, nameof(settingsStorage));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Check.ArgumentNotNull(destCard, nameof(destCard));
        ...
        public async Task UpdateStageSettingsAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            Check.ArgumentNotNull(innerCard, nameof(innerCard));
            Check.ArgumentNotNull(outerCard, nameof(outerCard));
            Check.ArgumentNotNull(krProcessCache, nameof(krProcessCache));
            ...
            StageNode y)
        {
            ...
            var xOrder = x.Row.Get<int>(KrConstants.Order);
            var yOrder = y.Row.Get<int>(KrConstants.Order);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var node in nodes.OrderByDescending(p => p.Row.Get<int>(KrConstants.Order)))
            {
                nodeStack.Push(node);
            }
            ...
            while (nodeStack.Count != 0)
            {
                ...
                var row = node.Row;
                var absoluteOrder = row.Get<int?>(KrConstants.Order) ?? 0;
                var hidden = false;
                var skip = false;
                ...
                visibleRow.Remove(KrConstants.Info);
                ...
                visibleRow[KrConstants.Order] = Int32Boxes.Box(shiftedOrder++);
