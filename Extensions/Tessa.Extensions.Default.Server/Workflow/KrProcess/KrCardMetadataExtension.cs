...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess
{
    ...
    public sealed class KrCardMetadataExtension : KrCardMetadataExtensionBase
    {
        ...
        {
            private readonly HashSet<Guid, CardMetadataContainerLink> forwardMapping = new(x => x.link.OriginalID);
            ...
            ICardMetadataExtensionContext context)
        {
            ...
            // получаем карточки (копии) с шаблонами этапов, которые будут далее внедрятся в расширяемые типы krTypes.
            ...
            ICardMetadataExtensionContext context)
        {
            ...
            // согласно логике работы InjectForms копируются блоки форм, значит их и нужно регистрировать.
            settingsCardType.Blocks.MakeGlobal(ctx, settingsCardType);
            ...
            CardType settingsCardType)
        {
            ...
            {
                TabCaption = settingsCardType.Caption,
                Name = settingsCardType.Name,
            };
        ...
        private InjectCardTypeInfo[] GetInjectableCards(
            ICardMetadataExtensionContext context) =>
            new[]
            {
                new InjectCardTypeInfo
                {
                    CardType = this.PrepareInjectableCardType(
