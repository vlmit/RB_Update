        ...
        CardGetFileVersionsExtension
    {
        ...
        public override Task AfterRequest(ICardGetFileVersionsExtensionContext context)
        {
            ...
            if (context.RequestIsSuccessful
                ...
                && info.TryGetValue(KrPermissionsHelper.FileAccessSettings.InfoKey, out var accessSettingObj)
                && accessSettingObj is int accessSetting)
            {
                var lastVersionNum = fileVersions.Max(x => x.Number);
                if (accessSetting == KrPermissionsHelper.FileAccessSettings.OnlyLastVersion)
                {
                    fileVersions.RemoveAll(x => x.Number != lastVersionNum);
                }
                else if (accessSetting == KrPermissionsHelper.FileAccessSettings.OnlyLastAndOwnVersions)
                {
